#!/usr/bin/env python3
"""
Generate spacer drawings for the Main Unit foam cavity (Rev 3.2).
...
Usage:
    python cad/main_unit/generate_spacers.py

Output:
    cad/main_unit/spacers/1_spacer_plan.dxf        + .svg
    cad/main_unit/spacers/2_spacer_cross_section.dxf + .svg
    cad/main_unit/spacers/3_corner_spacer_detail.dxf + .svg
    cad/main_unit/spacers/4_midwall_spacer_detail.dxf + .svg
"""
...
ELEC_ZONE_W = 320.0
ELEC_ZONE_D = STACK_D
ELEC_ZONE_H = max(2 * 76.0 + 30.0 + 80.0 + 50, STACK_H)  # MPPT panel + BMS + wiring = 312
...
# Corner spacer: L-shaped, sits in corner of Shell 2 outer surface
CORNER_LEG = 30.0       # length of each L-shape leg
CORNER_THICK = 12.0     # wall thickness of L-shape
SPACER_DEPTH = FOAM     # 30mm — fills foam gap exactly
SPACER_H = 25.0         # vertical height of each spacer block

# Mid-wall spacer: flat rectangular block on Shell 2 outer wall
MIDWALL_W = 30.0        # width along wall
MIDWALL_THICK = FOAM    # depth — fills foam gap
MIDWALL_H = SPACER_H   # same height as corner spacers

# Spacing between mid-wall spacers along each wall
MIDWALL_SPACING = 150.0

# Vertical placement: two rows of spacers at 1/3 and 2/3 of Shell 2 height
SPACER_Z1 = MID_H * 0.30
SPACER_Z2 = MID_H * 0.65

# Clamp bolt: M4 through Shell 1 + spacer + Shell 2 at each corner
CLAMP_HOLE = M4_HOLE
...
def corner_spacer_positions():
    """Return (x, y, quadrant) for each corner spacer in plan view.
    x, y = corner of Shell 2 outer surface.
    quadrant = (sx, sy) signs indicating which corner.
    """
    hw = MID_W / 2
    hd = MID_D / 2
    return [
...
def midwall_spacer_positions():
    """Return (cx, cy, orientation) for mid-wall spacers.
    orientation: 'h' = on horizontal wall (top/bottom), 'v' = on vertical wall (left/right).
    cx, cy = center of spacer on Shell 2 outer wall.
    """
    hw = MID_W / 2
    hd = MID_D / 2
    positions = []
    ...
    for i in range(n_w):
        ...
        positions.append((x,  hd, "h"))  # top wall
        positions.append((x, -hd, "h"))  # bottom wall

    # Along D axis (left and right walls)
    usable_d = MID_D - 2 * CORNER_LEG
    n_d = max(1, round(usable_d / MIDWALL_SPACING))
    for i in range(n_d):
        y = -hd + CORNER_LEG + (i + 0.5) * usable_d / n_d
        positions.append((-hw, y, "v"))  # left wall
        positions.append(( hw, y, "v"))  # right wall
...
def svg_line(x1, y1, x2, y2, color, dash=False):
    style = ' stroke-dasharray="4,3"' if dash else ""
    return (
        f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" '
        f'stroke="{color}" stroke-width="0.5"{style}/>\n'
    )
...
# ---------------------------------------------------------------------------
# DRAWING 1: Spacer Plan View (top-down, looking into foam cavity)
# ---------------------------------------------------------------------------
...
def generate_spacer_plan():
    ...
    # Corner spacers (L-shape footprint in plan)
    for cx, cy, (sx, sy) in corner_spacer_positions():
        # L-shape: two legs extending along each wall from the corner
        # Leg along W axis
        lx = cx if sx > 0 else cx - CORNER_LEG
        ly = cy if sy > 0 else cy - CORNER_THICK
        if sx > 0:
            lx = cx - CORNER_LEG
        else:
            lx = cx
        ...
        # Simplified: draw L as two overlapping rectangles
        # Horizontal leg
        ...
        # Clamp bolt hole (in the corner overlap zone)
    ...
    for cx, cy, orient in midwall_spacer_positions():
        if orient == "h":
            # On top/bottom wall — spacer extends into foam gap (along D axis)
            sy = 1 if cy > 0 else -1
            sx = -MIDWALL_W / 2
            sy_off = 0 if sy < 0 else -FOAM
            add_rect(msp, cx + sx, cy + sy_off, MIDWALL_W, FOAM, "SPACERS")
        else:
            # On left/right wall — extends into foam gap (along W axis)
            sx = 1 if cx > 0 else -1
    ...
    add_label(msp, -OUTER_W / 2 + 5, -OUTER_D / 2 - 15,
              f"SPACER PLAN — foam cavity between Shell 1 & Shell 2  (2 rows at Z={SPACER_Z1:.0f} & {SPACER_Z2:.0f}mm)",
              height=5)

    corners = corner_spacer_positions()
    midwalls = midwall_spacer_positions()
    add_label(msp, -OUTER_W / 2 + 5, -OUTER_D / 2 - 25,
              f"{len(corners)} corner spacers (L-shape + M4 bolt)  |  "
              f"{len(midwalls)} mid-wall spacers  |  x2 height levels = "
              f"{(len(corners) + len(midwalls)) * 2} total pieces",
              height=4)
    ...
    svg = svg_header(800, 600, (vb_x, vb_y, vb_w, vb_h))

    # Note: SVG uses same coord system (we flip at viewBox level if needed)
    # Shell 1 outer + inner
    ...
    for cx, cy, orient in midwall_spacer_positions():
        if orient == "h":
            ...
            sx = -MIDWALL_W / 2
            sy_off = 0 if sy < 0 else -FOAM
            svg += svg_rect(cx + sx, cy + sy_off, MIDWALL_W, FOAM,
    ...
    svg += svg_text(vb_x + 5, vb_y + vb_h - 10,
                    f"SPACER PLAN — Main Unit  |  {len(corners)} corner + {len(midwalls)} mid-wall  x2 levels",
                    size=6)
    svg += svg_footer()
...
# ---------------------------------------------------------------------------
# DRAWING 2: Spacer Cross-Section (front view showing spacers in foam)
# ---------------------------------------------------------------------------
...
def generate_spacer_cross_section():
    ...
    # Spacers at Z1 and Z2 on left and right walls
    for z in [SPACER_Z1, SPACER_Z2]:
        spacer_y = s2_y + z - SPACER_H / 2

        # Left wall spacer (corner spacer shown as section through L-leg)
    ...
    add_label(msp, -OUTER_W / 2 - 40, OUTER_H / 2,
              f"FOAM={FOAM:.0f}", "DIMENSIONS", height=3)

    add_label(msp, -OUTER_W / 2, -15,
              "FRONT CROSS-SECTION — spacers in foam cavity (corner spacers shown)",
              height=5)
    ...
    svg += svg_text(vb_x + 5, vb_y + vb_h - 10,
                    "FRONT CROSS-SECTION — spacers in foam cavity", size=6)
    svg += svg_footer()
...
# ---------------------------------------------------------------------------
# DRAWING 3: Corner Spacer Detail (individual part drawing)
# ---------------------------------------------------------------------------
...
def generate_corner_spacer_detail():
    ...
    add_label(msp, ox - 8, oy + CORNER_LEG / 2,
              f"{CORNER_LEG:.0f}", "DIMENSIONS", height=3)
    add_label(msp, ox + CORNER_THICK / 2 - 2, oy + CORNER_LEG + 3,
              f"{CORNER_THICK:.0f}", "DIMENSIONS", height=3)
    add_label(msp, ox + CORNER_THICK + 3, oy + CORNER_THICK / 2 - 2,
              f"M4 ({CLAMP_HOLE:.1f}mm)", "FASTENERS", height=3)

    add_label(msp, ox, oy + CORNER_LEG + 15,
              "PLAN VIEW", height=4)

    # --- Side view (XZ) ---
    sx, sy = CORNER_LEG + 60, 0
    add_rect(msp, sx, sy, CORNER_LEG, SPACER_H, "SPACERS")
    add_circle(msp, sx + CORNER_THICK / 2, sy + SPACER_H / 2,
               CLAMP_HOLE / 2, "FASTENERS")

    add_label(msp, sx, sy - 8, f"{CORNER_LEG:.0f}", "DIMENSIONS", height=3)
    add_label(msp, sx + CORNER_LEG + 3, sy + SPACER_H / 2 - 2,
              f"H={SPACER_H:.0f}", "DIMENSIONS", height=3)

    add_label(msp, sx, sy + SPACER_H + 15,
              "SIDE VIEW (leg profile)", height=4)

    # --- Front view (YZ) showing depth = FOAM ---
    fx, fy = 2 * (CORNER_LEG + 60), 0
    add_rect(msp, fx, fy, SPACER_DEPTH, SPACER_H, "SPACERS")
    add_circle(msp, fx + SPACER_DEPTH / 2, fy + SPACER_H / 2,
               CLAMP_HOLE / 2, "FASTENERS")

    add_label(msp, fx, fy - 8, f"DEPTH={SPACER_DEPTH:.0f} (=FOAM)", "DIMENSIONS", height=3)
    add_label(msp, fx + SPACER_DEPTH + 3, fy + SPACER_H / 2 - 2,
              f"H={SPACER_H:.0f}", "DIMENSIONS", height=3)

    add_label(msp, fx, fy + SPACER_H + 15,
              "FRONT VIEW (depth = foam gap)", height=4)

    # Title
    add_label(msp, 0, CORNER_LEG + 35,
              "CORNER SPACER — PETG  |  Qty: 8 (4 corners x 2 height levels)  |  "
              f"L-shape {CORNER_LEG:.0f}×{CORNER_LEG:.0f}×{SPACER_H:.0f}mm, "
              f"wall {CORNER_THICK:.0f}mm, depth {SPACER_DEPTH:.0f}mm",
              height=5)
    ...
    # --- SVG version ---
    total_w = 3 * (CORNER_LEG + 60)
    total_h = max(CORNER_LEG, SPACER_H) + 60
    margin = 20
    vb = (-margin, -margin, total_w + 2 * margin, total_h + 2 * margin)
    svg = svg_header(800, 400, vb)
    ...
    svg += svg_circle(ox + CORNER_THICK / 2, oy + CORNER_THICK / 2,
                      CLAMP_HOLE / 2, SVG_COLORS["FASTENERS"])
    svg += svg_text(ox, oy + CORNER_LEG + 15, "PLAN VIEW", size=5)
    ...
    svg += svg_circle(sx + CORNER_THICK / 2, sy + SPACER_H / 2,
                      CLAMP_HOLE / 2, SVG_COLORS["FASTENERS"])
    svg += svg_text(sx, sy + SPACER_H + 15, "SIDE VIEW", size=5)
    ...
    svg += svg_circle(fx + SPACER_DEPTH / 2, fy + SPACER_H / 2,
                      CLAMP_HOLE / 2, SVG_COLORS["FASTENERS"])
    svg += svg_text(fx, fy + SPACER_H + 15, "FRONT VIEW", size=5)

    svg += svg_text(0, total_h + margin - 5,
...
# ---------------------------------------------------------------------------
# DRAWING 4: Mid-Wall Spacer Detail (individual part drawing)
# ---------------------------------------------------------------------------
...
def generate_midwall_spacer_detail():
    ...
    midwalls = midwall_spacer_positions()
    qty = len(midwalls) * 2  # x2 height levels
    ...
    add_label(msp, ox + MIDWALL_W + 3, oy + MIDWALL_THICK / 2 - 2,
              f"{MIDWALL_THICK:.0f} (=FOAM)", "DIMENSIONS", height=3)
    add_label(msp, ox, oy + MIDWALL_THICK + 10, "PLAN VIEW", height=4)

    # --- Front view (WxH) ---
    fx, fy = MIDWALL_W + 60, 0
    add_rect(msp, fx, fy, MIDWALL_W, MIDWALL_H, "SPACERS")
    add_label(msp, fx, fy - 8, f"{MIDWALL_W:.0f}", "DIMENSIONS", height=3)
    add_label(msp, fx + MIDWALL_W + 3, fy + MIDWALL_H / 2 - 2,
              f"H={MIDWALL_H:.0f}", "DIMENSIONS", height=3)
    add_label(msp, fx, fy + MIDWALL_H + 10, "FRONT VIEW", height=4)

    # --- Side view (depth x H) ---
    sx, sy = 2 * (MIDWALL_W + 60), 0
    add_rect(msp, sx, sy, MIDWALL_THICK, MIDWALL_H, "SPACERS")
    add_label(msp, sx, sy - 8, f"DEPTH={MIDWALL_THICK:.0f}", "DIMENSIONS", height=3)
    add_label(msp, sx + MIDWALL_THICK + 3, sy + MIDWALL_H / 2 - 2,
              f"H={MIDWALL_H:.0f}", "DIMENSIONS", height=3)
    add_label(msp, sx, sy + MIDWALL_H + 10, "SIDE VIEW", height=4)

    add_label(msp, 0, max(MIDWALL_THICK, MIDWALL_H) + 30,
              f"MID-WALL SPACER — PETG  |  Qty: {qty} ({len(midwalls)} positions x 2 levels)  |  "
              f"{MIDWALL_W:.0f}×{MIDWALL_THICK:.0f}×{MIDWALL_H:.0f}mm  |  simple block",
              height=5)
    ...
    # --- SVG version ---
    total_w = 3 * (MIDWALL_W + 60)
    total_h = max(MIDWALL_THICK, MIDWALL_H) + 50
    margin = 20
    vb = (-margin, -margin, total_w + 2 * margin, total_h + 2 * margin)
    svg = svg_header(800, 300, vb)

    svg += svg_rect(ox, oy, MIDWALL_W, MIDWALL_THICK,
                    SVG_COLORS["SPACERS"], fill="rgba(204,68,204,0.3)")
    svg += svg_text(ox, oy + MIDWALL_THICK + 12, "PLAN VIEW", size=5)

    svg += svg_rect(fx, fy, MIDWALL_W, MIDWALL_H,
                    SVG_COLORS["SPACERS"], fill="rgba(204,68,204,0.3)")
    svg += svg_text(fx, fy + MIDWALL_H + 12, "FRONT VIEW", size=5)

    svg += svg_rect(sx, sy, MIDWALL_THICK, MIDWALL_H,
                    SVG_COLORS["SPACERS"], fill="rgba(204,68,204,0.3)")
    svg += svg_text(sx, sy + MIDWALL_H + 12, "SIDE VIEW", size=5)
...
def main():
    ...
    print("Generating Main Unit Spacer Drawings (Rev 3.2)...")
